  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full 
        ...
        bg-white shadow-md z-50 h-16 flex items-center justify-between px-4 md:px-6`}
    >
      {/* Sidebar button for mobile & tablet */}
      ...
      {/* Left section: Search + Mega Menu + Apps */}
      <div className="flex-1 flex items-center gap-2 md:gap-3 w-full lg:w-auto">
        {/* Search - show on tablet+ */}
        <div className="hidden md:flex items-center bg-gray-100 rounded-full px-3 py-1 gap-2 shadow-sm w-full lg:w-auto">
        ...
        <div ref={megaRef} className="hidden lg:flex relative">
          ...
          >
            Mega Menu
            <IoChevronDown className={`transition-transform duration-300 ${megaOpen ? "rotate-180" : ""}`} />
          </button>
        </div>
        ...
        <div ref={appsRef} className="hidden lg:flex relative">
          ...
          >
            Apps
            <IoChevronDown className={`transition-transform duration-300 ${appsOpen ? "rotate-180" : ""}`} />
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <div className="flex items-center gap-2 lg:hidden">
          ...
          <div className="relative flex items-center cursor-pointer">
            <GrLanguage className="text-lg md:text-xl text-gray-600" />
          </div>
        ...
        {/* Tablet + Laptop: full icons on right */}
        <div className="hidden lg:flex items-center gap-2 md:gap-3">
          ...
          {/* Language dropdown */}
          <div ref={langRef} className="relative flex items-center border-l border-gray-300 pl-2 md:pl-3 cursor-pointer">
            <button
            ...
            >
              ...
              <IoChevronDown className={`text-gray-600 text-sm md:text-base transition-transform duration-300 ${langOpen ? "rotate-180" : ""}`} />
            </button>
            {langOpen && (
              <div className="absolute right-0 top-10 bg-white shadow-lg rounded-lg p-2 w-32 z-50">
                {languages.map((lang) => (
                  <div key={lang} className="cursor-pointer hover:bg-gray-100 px-2 py-1 rounded text-sm">
                    {lang}
                  </div>
                ))}
              </div>
          ...
          {/* User profile */}
          <div ref={userRef} className="relative flex items-center gap-2 cursor-pointer border-l border-gray-300 pl-2 md:pl-3" onClick={() => setUserOpen(!userOpen)}>
            <div className="w-10 h-10 overflow-hidden rounded-full">
              <img src={img} alt="Zarak Khan" className="w-full h-full object-cover scale-110" />
            </div>
            ...
            <IoChevronDown className={`text-gray-600 ${userOpen ? "rotate-180" : ""}`} />
            ...
            {userOpen && (
              <div className="absolute right-0 top-14 bg-white shadow-lg rounded-lg w-52 z-50 p-2">
                ...
                <ul className="space-y-2 mt-2">
                  {userMenu.map((item) => (
                    <li key={item.label} className="flex items-center gap-2 cursor-pointer hover:bg-gray-100 p-2 rounded text-sm">
                      {item.icon}
